...
export class RouteInputComponent implements OnInit {
	...
	this.timeDateForm.valueChanges.subscribe((timeDate) => {
		...
		console.log(this.timeDateForm.controls.timeInput.errors && this.timeDateForm.controls.timeInput.errors.required,
			    this.timeDateForm.controls.timeInput.errors.required === true,
			    !this.timeDateForm.controls.timeInput.errors.required === true
		)
	    }
	});
    ...
    addTimeDate(timeDate: Date) {
	this.dataService.setDepartureTime(timeDate);
    }
    
    printRoute(route: Date | boolean) {
	console.log(`route: ${route} | route object: ${JSON.stringify(route)}`);
	if (typeof route === "object") {
	    this.validRoute = true;
	    this.addTimeDate(route);
	} else {
